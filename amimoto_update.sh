...
[ -f /usr/bin/python2.7 ] && /usr/sbin/alternatives --set python /usr/bin/python2.7
if [ ! -f /opt/local/amimoto-managed.json ]; then
  /opt/chef/bin/chef-solo -c /opt/local/solo.rb -j /opt/local/amimoto.json -l error
else
  /opt/chef/bin/chef-solo -c /opt/local/solo.rb -j /opt/local/amimoto-managed.json -l error
fi
yum -y update
/opt/local/provision
